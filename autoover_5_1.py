...
class NBACompleteSystem:
    ...
    def predict_complete_game_flow(self, game_data, manual_base_line=None):
        ...
        momentum_factor = 1.0  # Impostiamo il fattore di momentum a 1.0 (valore di default)
        ...
        try:
            ...
            if hasattr(self, 'momentum_predictor') and self.momentum_predictor is not None:
                ...
                try:
                    ...
                    if 'injury_reports' in game_data:
                        home_injury_data = None
                        away_injury_data = None
                        
                        # Cerca i dati nei vari posti possibili
                        for key in game_data['injury_reports']:
                            if isinstance(game_data['injury_reports'][key], dict):
                                if 'team_id' in game_data['injury_reports'][key]:
                                    if game_data['injury_reports'][key]['team_id'] == home_team_id:
                                        home_injury_data = game_data['injury_reports'][key]
                                    elif game_data['injury_reports'][key]['team_id'] == away_team_id:
                                        away_injury_data = game_data['injury_reports'][key]
                        
                        # Estrai i roster
                        home_roster_list = home_injury_data.get('roster_data', []) if home_injury_data else []
                        away_roster_list = away_injury_data.get('roster_data', []) if away_injury_data else []
                    
                    # Se i roster sono vuoti, prova a recuperarli in altro modo
                    if not home_roster_list and home_team_id:
                        print(f"\n🔍 Recupero roster per {home_team_name} (ID: {home_team_id})...")
                        home_roster = self._get_team_roster(home_team_id, home_team_name)
                    
                    if not away_roster_list and away_team_id:
                        print(f"🔍 Recupero roster per {away_team_name} (ID: {away_team_id})...")
                        away_roster = self._get_team_roster(away_team_id, away_team_name)
    ...
    def _calculate_momentum_impact(self, home_roster, away_roster):
        ...
        try:
            ...
            # Debug: Print roster column names and first few rows
            print("\n📋 [DEBUG] Home Roster Columns:", home_roster.columns.tolist() if not home_roster.empty else "Empty DataFrame")
            print("📋 [DEBUG] Away Roster Columns:", away_roster.columns.tolist() if not away_roster.empty else "Empty DataFrame")
            
            if not home_roster.empty:
                print("\n🏠 [DEBUG] Home Roster First 3 Rows:")
                print(home_roster.head(3).to_string())
                
            if not away_roster.empty:
                print("\n✈️ [DEBUG] Away Roster First 3 Rows:")
                print(away_roster.head(3).to_string())
            ...
            result = {
                ...
                'home_players_count': len(home_roster) if home_roster is not None else 0,
                'away_players_count': len(away_roster) if away_roster is not None else 0,
    ...
    def _get_team_roster(self, team_id, team_name):
        """
        Recupera il roster di una squadra con le informazioni sugli infortuni.
        ...
        Returns:
            pd.DataFrame: DataFrame con i giocatori del roster e le informazioni sugli infortuni
        """
        try:
            print(f"🔍 [ROSTER] Recupero roster per {team_name} (ID: {team_id})...")
            
            # Verifica se abbiamo un'istanza di InjuryReporter
            ...
            # Converti team_id in intero se non è None
            if team_id is not None:
                try:
                    team_id = int(team_id)
                except (ValueError, TypeError):
                    print(f"⚠️ [WARNING] Formato team_id non valido: {team_id}")
                    team_id = None
            
            # Se non abbiamo un team_id valido, prova a recuperarlo dal nome della squadra
            if team_id is None and team_name:
                try:
                    team_info = self.data_provider.find_team_by_name(team_name)
                    if team_info and 'id' in team_info:
                        team_id = int(team_info['id'])
                        print(f"ℹ️ [INFO] Trovato ID {team_id} per la squadra {team_name}")
                except Exception as e:
                    print(f"⚠️ [WARNING] Impossibile trovare ID per la squadra {team_name}: {str(e)}")
            
            # Se ancora non abbiamo un team_id, restituisci un DataFrame vuoto
            if team_id is None:
                print(f"❌ [ERROR] Impossibile determinare l'ID della squadra per {team_name}")
                return pd.DataFrame(columns=['PLAYER_ID', 'PLAYER_NAME', 'POSITION', 'MIN', 'TEAM_ID', 'ROTATION_STATUS', 'INJURY_STATUS'])
            
            # Recupera il roster con gli infortuni
            print(f"   [ROSTER] Recupero dati roster e infortuni per {team_name} (ID: {team_id})...")
            ...
            # Crea il DataFrame con i dati mappati
            roster_data = []
            for i, player in enumerate(roster):
                ...
                for old_key, new_key in column_mapping.items():
                    mapped_player[new_key] = player.get(old_key)
                
                # Gestisci i valori mancanti
                if 'PLAYER_NAME' not in mapped_player or not mapped_player['PLAYER_NAME']:
                    first_name = player.get('first_name', '')
                    last_name = player.get('last_name', '')
                    mapped_player['PLAYER_NAME'] = f"{first_name} {last_name}".strip()
                
                # Imposta lo status di rotazione (Starter/Bench)
                mapped_player['ROTATION_STATUS'] = 'Starter' if i < 5 else 'Bench'
                
                # Gestisci lo status infortunio
                if 'INJURY_STATUS' not in mapped_player or not mapped_player['INJURY_STATUS'] or mapped_player['INJURY_STATUS'] == 'N/A':
                    mapped_player['INJURY_STATUS'] = 'Active'
                
                # Converti i minuti in numerico
                try:
                    if 'MIN' in mapped_player and mapped_player['MIN'] is not None:
                        if isinstance(mapped_player['MIN'], str) and ':' in mapped_player['MIN']:
                            # Formato MM:SS -> converti in minuti decimali
                            minutes, seconds = map(int, mapped_player['MIN'].split(':'))
                            mapped_player['MIN'] = minutes + (seconds / 60.0)
                        else:
                            mapped_player['MIN'] = float(mapped_player['MIN'])
                    else:
                        mapped_player['MIN'] = 0.0
                except (ValueError, TypeError):
                    mapped_player['MIN'] = 0.0
                
                # Assicurati che TEAM_ID sia impostato
                if 'TEAM_ID' not in mapped_player or not mapped_player['TEAM_ID']:
                    mapped_player['TEAM_ID'] = team_id
                
                roster_data.append(mapped_player)
            ...
            for col in required_columns:
                if col not in roster_df.columns:
                    print(f"⚠️ [ROSTER_DEBUG] Colonna mancante dopo mappatura: {col}")
                    roster_df[col] = None
            
            # Log di debug
            print(f"✅ [ROSTER] Trovati {len(roster_df)} giocatori per {team_name}")
            if not roster_df.empty:
                player_names = roster_df['PLAYER_NAME'].fillna('Sconosciuto').head(3).tolist()
                print(f"   • Primi 3 giocatori: {', '.join(map(str, player_names))}")
                
                starters = roster_df[roster_df['ROTATION_STATUS'] == 'Starter']['PLAYER_NAME'].head(5).tolist()
                starters = [str(name) if name is not None else 'Sconosciuto' for name in starters]
                print(f"   • Starter: {starters}")
                
                avg_minutes = roster_df['MIN'].mean() if 'MIN' in roster_df.columns else 0
                print(f"   • Minuti medi: {avg_minutes:.1f} per giocatore")
            ...
            empty_df = pd.DataFrame(columns=['PLAYER_ID', 'PLAYER_NAME', 'POSITION', 'MIN', 'TEAM_ID', 'ROTATION_STATUS', 'INJURY_STATUS'])
