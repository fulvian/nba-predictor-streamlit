...
class NBACompleteSystem:
    ...
    def analyze_game(self, game_details, central_line=None):
        ...
        # Ottieni la mappa degli infortuni per tutta la lega
        league_injuries_map = self.injury_reporter._fetch_injuries_from_rotowire()
        
        # Usa il metodo corretto con la mappa degli infortuni
        # Tentativo di recuperare i roster con gestione degli errori
        try:
            ...
            roster_available = not (home_roster_df.empty or away_roster_df.empty)
            
            if not roster_available:
                print("⚠️ Impossibile recuperare i roster completi. L'analisi continuerà con impatto infortuni pari a 0.")
                # Crea dataframe vuoti con le colonne necessarie per evitare errori
                home_roster_df = pd.DataFrame(columns=['PLAYER_ID', 'name', 'injury_status'])
                away_roster_df = pd.DataFrame(columns=['PLAYER_ID', 'name', 'injury_status'])
                
        except Exception as e:
            print(f"⚠️ Errore nel recupero dei roster: {str(e)}. L'analisi continuerà con impatto infortuni pari a 0.")
            # Crea dataframe vuoti con le colonne necessarie per evitare errori
            home_roster_df = pd.DataFrame(columns=['PLAYER_ID', 'name', 'injury_status'])
            away_roster_df = pd.DataFrame(columns=['PLAYER_ID', 'name', 'injury_status'])
        ...
        # Se i dataframe dei roster sono vuoti, salta il calcolo dell'impatto
        if home_roster_df.empty or away_roster_df.empty:
            print("   -> Impatto Infortuni: Dati non disponibili. Utilizzato valore di default: 0.00 punti")
        else:
            try:
                ...
                home_injury_pts = home_impact_result.get('total_impact', 0.0) if home_impact_result else 0.0
                away_injury_pts = away_impact_result.get('total_impact', 0.0) if away_impact_result else 0.0
