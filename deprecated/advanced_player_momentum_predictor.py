...
class AdvancedPlayerMomentumPredictor:
    ...
    def _calculate_advanced_offensive_metrics(self, game_logs, player_season_avg=None):
        ...
        for _, game in games.iterrows():
            ...
            # USG_PCT non è disponibile nei game logs NBA API, calcoliamo approssimazione
            usage = float(game.get('USG_PCT', game.get('USAGE_PCT', 0)))
            if usage == 0:  # Sempre 0 perché USG_PCT non esiste nei game logs
                min_played = float(game.get('MIN', 1))
                # Approssimazione Usage Rate = (FGA + 0.44*FTA + TOV) * (Team Minutes / 5) / (Minutes * (Team FGA + 0.44*Team FTA + Team TOV))
                # Semplificazione: (FGA + 0.44*FTA) / Minutes * scaling factor
                if min_played > 0:
                    usage = ((fga + 0.44 * fta) / min_played) * 10  # Scaling factor approssimativo
                else:
                    usage = 0
    ...
    def predict_team_momentum_impact_advanced(self, team_roster_df):
        ...
        for _, player_series in team_roster_df.iterrows():
            # Debug: stampa la struttura del player_series
            #if len(team_contributions) == 0:  # Solo per il primo giocatore
            #    print(f"[DEBUG] Player series keys: {list(player_series.index) if hasattr(player_series, 'index') else 'N/A'}")
            #    print(f"[DEBUG] Player series sample: {dict(player_series.head(5)) if hasattr(player_series, 'head') else player_series}")
            
            # Estrai player_id gestendo diversi formati
            player_id = self._safe_get(player_series, 'PLAYER_ID')
            if player_id is None:
                player_id = self._safe_get(player_series, 'id')  # Formato alternativo
            
            # Converti da Series a valore se necessario - FIX CRITICO!
            if hasattr(player_id, 'iloc'):
                player_id = player_id.iloc[0] if len(player_id) > 0 else None
            elif hasattr(player_id, 'values'):
                player_id = player_id.values[0] if len(player_id.values) > 0 else None
                
            # Converti a intero se è ancora un oggetto pandas
            ...
            if player_id is not None:
                try:
                    ...
                    print(f"⚠️ Player ID non convertibile: {player_id}")
                    player_id = None
                
            # Estrai player_name gestendo diversi formati  
            player_name = self._safe_get(player_series, 'PLAYER_NAME')
            if player_name is None:
                player_name = self._safe_get(player_series, 'full_name')  # Formato alternativo
            if player_name is None:
            ...
            try:
                ...
                position = self._safe_get(player_series, 'POSITION', 'UNKNOWN')
                if position is None:
                    position = 'UNKNOWN'
                pos_weights = self.position_impact_weights.get(position)
                
                if pos_weights is None:
                    # Default weights se la posizione non è riconosciuta
                    pos_weights = {'offensive': 1.0, 'defensive': 1.0, 'usage_sensitivity': 1.0}
                    
                rotation_status = self._safe_get(player_series, 'ROTATION_STATUS', 'BENCH')
                if rotation_status is None:
                    rotation_status = 'BENCH'
                rotation_weight = self._get_rotation_weight(rotation_status)
