...
def setup_environment():
    ...
    if not venv_path:
        logger.error("❌ No virtual environment found. Please create one first.")
        return False
    
    # Activate virtual environment
    activate_script = venv_path / "bin" / "activate"
    if not activate_script.exists():
        logger.error(f"❌ Virtual environment activation script not found: {activate_script}")
        return False
...
def main():
    ...
    try:
        ...
        if not services_started:
            logger.error("❌ No services were started")
            sys.exit(1)
