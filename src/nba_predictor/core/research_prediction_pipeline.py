...
from ..explainability.shap_explainer import calculate_local_shap_values
...
class ResearchPredictionPipeline:
    ...
    def _validate_paths(self) -> None:
        ...
        missing_files = [f for f in required_files if not (self.data_path / f).exists()]

        if missing_files:
            logger.warning(
                "Some data files missing", extra={"missing_files": missing_files}
            )
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            ...
            if nba_data_file.exists():
                ...
                logger.info(
                    f"Loaded {len(games_df)} real NBA games from {nba_data_file}"
                )
    ...
    def _create_sample_data(self) -> pd.DataFrame:
        ...
        sample_data["total_score"] = (
            sample_data["team1_score"] + sample_data["team2_score"]
        )
        sample_data["team1_two_pointers_made"] = (
            sample_data["team1_field_goals_made"]
            - sample_data["team1_three_pointers_made"]
        )
        sample_data["team2_two_pointers_made"] = (
            sample_data["team2_field_goals_made"]
            - sample_data["team2_three_pointers_made"]
        )
    ...
    def train_model(
        ...
        try:
            ...
            logger.info("Model training completed successfully", extra=self.metrics)
    ...
    def _initialize_shap_explainer(self, X_background: np.ndarray) -> None:
        ...
        try:
            ...
            self.shap_explainer = create_nba_shap_explainer(
                self.model, background_df, model_output="raw"
            )
            ...
            logger.warning(
                "Failed to initialize SHAP explainer", extra={"error": str(e)}
            )
    ...
    def _create_default_features(
        ...
        try:
            ...
            nba_data_path = (
                ...
                / "data"
                / "nba_data_with_mu_sigma_for_ml.csv"
            )

            if not nba_data_path.exists():
                logger.warning(
                    f"NBA data file not found at {nba_data_path}, using league averages"
                )
            ...
            if df.empty:
                logger.warning(
                    "No valid data found in NBA data file, using league averages"
                )
                return self._get_league_averages_features()
            ...
            features["team1_two_pointers_made"] = (
                features["team1_field_goals_made"]
                - features["team1_three_pointers_made"]
            )
            features["team2_two_pointers_made"] = (
                features["team2_field_goals_made"]
                - features["team2_three_pointers_made"]
            )
            features["team1_defensive_rebounds"] = (
                features["team1_rebounds"] - features["team1_offensive_rebounds"]
            )
            features["team2_defensive_rebounds"] = (
                features["team2_rebounds"] - features["team2_offensive_rebounds"]
            )
            ...
            team_adjustments = self._get_data_driven_team_adjustments(
                team1_name, team2_name, df
            )
    ...
    def _get_data_driven_team_adjustments(
        self, team1_name: str, team2_name: str, df: pd.DataFrame
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            # Define score_std for use in adjustments
            score_std = (
            ...
            # Dynamic Team Analysis (No more hardcoded lists)
            # We calculate the team's actual performance relative to the league from the data

            # Calculate league average ORtg
            league_avg_ortg = (
                df["HOME_ORtg"].mean() if "HOME_ORtg" in df.columns else 114.0
            )
            ortg_std = df["HOME_ORtg"].std() if "HOME_ORtg" in df.columns else 10.0

            # Helper to get team rating
            def get_team_rating(team_name):
                # Filter games where team played
                team_games_home = df[df["home_team"] == team_name]
                team_games_away = df[df["away_team"] == team_name]

                if team_games_home.empty and team_games_away.empty:
                    return league_avg_ortg  # Default to average if no data

                # Calculate average rating
                home_rating = (
                    team_games_home["HOME_ORtg"].mean()
                    if not team_games_home.empty
                    else 0
                )
                away_rating = (
                    team_games_away["AWAY_ORtg"].mean()
                    if not team_games_away.empty
                    else 0
                )
                ...
                avg_rating = (
                    (home_rating * len(team_games_home))
                    + (away_rating * len(team_games_away))
                ) / total_games

                return avg_rating

            # Calculate ratings
            team1_rating = get_team_rating(team1_name)
            team2_rating = get_team_rating(team2_name)

            # Calculate Z-scores (how many std devs above/below average)
            team1_z = (team1_rating - league_avg_ortg) / ortg_std if ortg_std > 0 else 0
            team2_z = (team2_rating - league_avg_ortg) / ortg_std if ortg_std > 0 else 0

            # Apply adjustments based on Z-scores
            # High performance (z > 0.5): Boost score and efficiency
            # Low performance (z < -0.5): Reduce score and efficiency

            # Team 1 Adjustment
            if team1_z > 0.5:
                # High performing team
                adjustments["team1_score"] += (
                    score_std * 0.3 * min(team1_z, 2.0)
                )  # Cap boost
                adjustments["efg_pct"] += 0.01 * min(team1_z, 2.0)
            elif team1_z < -0.5:
                # Low performing team
                adjustments["team1_score"] += (
                    score_std * 0.2 * max(team1_z, -2.0)
                )  # Negative boost
                adjustments["efg_pct"] += 0.008 * max(team1_z, -2.0)

            # Team 2 Adjustment
            if team2_z > 0.5:
                adjustments["team2_score"] += score_std * 0.3 * min(team2_z, 2.0)
                adjustments["efg_pct"] += 0.01 * min(team2_z, 2.0)
            elif team2_z < -0.5:
                adjustments["team2_score"] += score_std * 0.2 * max(team2_z, -2.0)
                adjustments["efg_pct"] += 0.008 * max(team2_z, -2.0)

            logger.info(
                f"Calculated dynamic adjustments for {team1_name} (Rating: {team1_rating:.1f}, Z: {team1_z:.2f}) "
                f"and {team2_name} (Rating: {team2_rating:.1f}, Z: {team2_z:.2f})"
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.model is not None:
            ...
            if hasattr(self.model, "estimators_") and hasattr(
                self.model.estimators_, "__iter__"
            ):
                try:
                    info["base_models"] = [
