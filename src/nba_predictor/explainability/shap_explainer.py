...
def create_nba_shap_explainer(
    model: Any, X_background: pd.DataFrame, model_output: str = "raw"
) -> "shap.Explainer":
    """
    ...
    if not _shap_available or shap is None:
        logger.error("SHAP not available", extra={"error": SHAP_IMPORT_ERROR})
        raise ImportError(
...
def validate_explainer_inputs(
    model: Any, X_background: pd.DataFrame, model_output: str
) -> None:
    """
    ...
    if len(X_background) < 10:
        raise ValueError(
            "Background dataset should have at least 10 samples for stable explanations"
        )

    if model_output not in ["raw", "probability", "log_odds"]:
        raise ValueError(
            "model_output must be one of: 'raw', 'probability', 'log_odds'"
        )
...
def create_tree_explainer(
    ...
    if "stackingregressor" in model_name:
        ...
        if hasattr(model, "final_estimator_"):
            ...
            raise ValueError(
                "StackingRegressor must be fitted before creating explainer"
            )
...
def calculate_global_shap_values(
    explainer: "shap.Explainer", X_test: pd.DataFrame
) -> "shap.Explanation":
    """
...
def calculate_local_shap_values(
    ...
    try:
        ...
        logger.error(
            ...
            extra={"instance_shape": single_instance.shape, "error": str(e)},
        )
        raise ValueError(f"Failed to calculate local SHAP values: {e}") from e


def get_feature_importance_from_shap(
    shap_values: "shap.Explanation", feature_names: Optional[List[str]] = None
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if feature_names is None:
            raise ValueError(
                "Feature names must be provided or available in shap_values"
            )
        ...
        importance_df = pd.DataFrame(
            {"feature": feature_names, "mean_abs_shap_value": mean_shap_values}
        ...
        logger.error("Feature importance extraction failed", extra={"error": str(e)})
...
def create_shap_summary_plot(
    ...
    try:
        ...
        plt.title(
            f"NBA Prediction Feature Importance ({plot_type.capitalize()} Plot)",
            fontsize=14,
            pad=20,
        )
        plt.tight_layout()
        ...
        logger.error(
            ...
            extra={"plot_type": plot_type, "error": str(e)},
        )
        raise ValueError(f"Failed to create SHAP summary plot: {e}") from e


def create_waterfall_plot(
    shap_values: "shap.Explanation", instance_idx: int = 0, show_plot: bool = False
) -> Optional[Any]:
    """
    ...
    try:
        ...
        plt.title(
            f"NBA Prediction Explanation - Instance {instance_idx}", fontsize=14, pad=20
        )
        plt.tight_layout()
        ...
        logger.info(
            ...
            extra={
                ...
                "base_value": float(shap_values[instance_idx].base_values)
                if hasattr(shap_values[instance_idx], "base_values")
                else None,
            },
        )
        ...
        logger.error(
            ...
            extra={"instance_idx": instance_idx, "error": str(e)},
        )
...
def generate_nba_explanation_report(
    ...
    try:
        ...
        feature_importance = get_feature_importance_from_shap(
            shap_values, X_test.columns.tolist()
        )
        ...
        report = {
            ...
            "feature_importance": {
                "top_features": feature_importance.head(top_features).to_dict(
                    "records"
                ),
                "total_features": len(feature_importance),
            },
            "shap_summary": {
                "mean_absolute_shap": float(np.abs(shap_values.values).mean())
                if hasattr(shap_values, "values")
                else None,
                "max_shap_value": float(np.abs(shap_values.values).max())
                if hasattr(shap_values, "values")
                else None,
                "feature_count": len(X_test.columns),
            },
            "data_info": {
                "background_size": getattr(explainer, "background_data", None)
                is not None
                and len(getattr(explainer, "background_data", [])),
        ...
        logger.error(
            "NBA explanation report generation failed", extra={"error": str(e)}
        )
