...
class LegacyRiskManager:
    ...
    QUOTE_SCHEMA = [
        ...
        (-6.0, 1.48, 2.55), (-5.5, 1.50, 2.50), (-5.0, 1.53, 2.45), (-4.5, 1.55, 2.40),
        (-4.0, 1.57, 2.35), (-3.5, 1.60, 2.30), (-3.0, 1.62, 2.28), (-2.5, 1.65, 2.25),
        (-2.0, 1.68, 2.20), (-1.5, 1.70, 2.15), (-1.0, 1.73, 2.10), (-0.5, 1.75, 2.05),
        (0.0, 1.90, 1.90),  # Linea centrale
        (0.5, 2.05, 1.75), (1.0, 2.10, 1.73), (1.5, 2.15, 1.70), (2.0, 2.20, 1.68),
        (2.5, 2.25, 1.65), (3.0, 2.28, 1.62), (3.5, 2.30, 1.60), (4.0, 2.35, 1.57),
        (4.5, 2.40, 1.55), (5.0, 2.45, 1.53), (5.5, 2.50, 1.50), (6.0, 2.55, 1.48),
        (6.5, 2.60, 1.45), (7.0, 2.65, 1.43), (7.5, 2.75, 1.40), (8.0, 2.85, 1.38)
    ]
    ...
    def update_bankroll_from_bet(self, bet_result: Dict[str, Any], actual_total: Optional[float] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def calculate_advanced_stake(self, edge: float, estimated_prob: float, odds: float,
                                bankroll: Optional[float] = None, quality_data: Optional[Dict] = None) -> float:
        """
        🚀 ADVANCED GRANULAR STAKE CALCULATOR - Replica Esatta Legacy
        ...
        - bankroll: Bankroll disponibile (usa corrente se None)
        - quality_data: Dati di qualità dal quality_score

        Returns:
            Stake raccomandato in Euro
        """
        ...
        # Usa bankroll corrente se non specificato
        if bankroll is None:
            bankroll = self.current_bankroll

        # Estrai i dati di qualità se disponibili
        if quality_data is None:
            quality_data = self.calculate_quality_score(edge, estimated_prob, odds)
    ...
    def calculate_quality_score(self, edge: float, estimated_prob: float, odds: float) -> Dict[str, Any]:
        """
    ...
    def analyze_betting_opportunities(self, distribution: Dict[str, Any], odds_list: List[Dict] = None,
                                    central_line: float = None, bankroll: float = None) -> List[Dict[str, Any]]:
        """
        ...
        for odds_item in odds_list:
            ...
            quality_under = self.calculate_quality_score(edge_under, prob_under, under_odds)

            # Valuta se sono VALUE bets
            is_value_over = edge_over > 0 and prob_over >= 0.50
            is_value_under = edge_under > 0 and prob_under >= 0.50

            # Calcola stake usando algoritmo legacy
            stake_over = self.calculate_advanced_stake(edge_over, prob_over, over_odds, bankroll, quality_over) if is_value_over else 0
            stake_under = self.calculate_advanced_stake(edge_under, prob_under, under_odds, bankroll, quality_under) if is_value_under else 0
    ...
    def calculate_optimal_bet(self, opportunities: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_bankroll_status(self) -> Dict[str, Any]:
        ...
        """
        pending_bets = self._load_pending_bets()
        pending_stake = sum(bet.get('bet_data', {}).get('stake', 0) for bet in pending_bets if bet.get('status') == 'pending')

        return {
            'current_bankroll': self.current_bankroll,
            'pending_stake': pending_stake,
            'available_bankroll': self.current_bankroll - pending_stake,
            'pending_bets_count': len([b for b in pending_bets if b.get('status') == 'pending']),
