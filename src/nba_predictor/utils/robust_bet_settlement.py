...
class NBATeamMapper:
    ...
    TEAM_MAPPINGS = {
        ...
        'Brooklyn Nets': 'BRK', 'Nets': 'BRK', 'Brooklyn': 'BRK',
        'New York Knicks': 'NYK', 'Knicks': 'NYK', 'New York': 'NYK',
        'Philadelphia 76ers': 'PHI', 'Sixers': 'PHI', 'Philadelphia': 'PHI', '76ers': 'PHI',
        'Toronto Raptors': 'TOR', 'Raptors': 'TOR', 'Toronto': 'TOR',

        # Eastern Conference - Central
        'Chicago Bulls': 'CHI', 'Bulls': 'CHI', 'Chicago': 'CHI',
        'Cleveland Cavaliers': 'CLE', 'Cavaliers': 'CLE', 'Cleveland': 'CLE', 'Cavs': 'CLE',
        ...
        'Minnesota Timberwolves': 'MIN', 'Timberwolves': 'MIN', 'Minnesota': 'MIN', 'T-Wolves': 'MIN',
        'Oklahoma City Thunder': 'OKC', 'Thunder': 'OKC', 'Oklahoma City': 'OKC',
        'Portland Trail Blazers': 'POR', 'Trail Blazers': 'POR', 'Portland': 'POR', 'Blazers': 'POR',
        ...
        'Dallas Mavericks': 'DAL', 'Mavericks': 'DAL', 'Dallas': 'DAL', 'Mavs': 'DAL',
    ...
    @classmethod
    def calculate_match_confidence(cls, bet_home: str, bet_away: str, api_home: str, api_away: str) -> float:
...
class NBABoxscoreAPI:
            ...
            if 'PlayerStats' in data and data['PlayerStats']:
                ...
                if player_stats:
                    ...
                    if home_score > 0 and away_score > 0:
                        logger.info(f"✅ Boxscore API found: {game_id} → {away_score}-{home_score}")
                        ...
                        if rs['name'] == 'Scoreboard':
                            for row in rs['rowSet']:
                                if str(row[0]) == game_id:  # GAME_ID match
                                    ...
                                    if 'Final' in game_status and home_score > 0 and away_score > 0:
                                        logger.info(f"✅ Fallback API found: {game_id} → {away_score}-{home_score}")
                                        return (home_score, away_score)
        except Exception as e:
    ...
    def find_game_by_teams_and_date(self, home_team: str, away_team: str, game_date: datetime) -> Optional[str]:
        """
        Trova NBA game ID usando team names e data.

        Args:
            home_team: Nome team home
            away_team: Nome team away
            game_date: Data della partita

        Returns:
            NBA game ID o None se non trovato
        """
        try:
            ...
            if 'GameHeader' in data and data['GameHeader']:
                for game in data['GameHeader']:
                    api_home = game.get('TEAM_NAME_HOME', '')
                    api_away = game.get('TEAM_NAME_AWAY', '')
                    ...
                    if confidence >= 0.8:  # High confidence threshold
                        logger.info(f"✅ Found NBA game: {home_team} vs {away_team} → {api_game_id} (confidence: {confidence})")
                        return api_game_id

        except Exception as e:
            logger.debug(f"Failed to find game by teams: {e}")
...
class RobustBetSettlement:
        ...
        for match in game_matches:
            ...
            try:
                ...
                if final_score:
                    ...
                    self.logger.info(f"✅ Got final score for {match.bet_id}: {final_score[1]}-{final_score[0]}")
                else:
                    self.logger.warning(f"⚠️ No final score found for game {match.nba_game_id}")

                settled_matches.append(match)

            except Exception as e:
                self.logger.error(f"❌ Error getting final score for {match.bet_id}: {e}")
        ...
        for match in game_matches:
            try:
                ...
                if settled_count > 0:
                    ...
                    self.logger.info(f"✅ Settled {settled_count} bets for {match.bet_id}: {away_score}-{home_score}")
                else:
                    settlement_report['failed_settlements'] += 1
                    settlement_report['details'].append({
                        'bet_id': match.bet_id,
    ...
    def execute_robust_settlement(self) -> Dict[str, any]:
        ...
        success_rate = (settlement_report['successful_settlements'] / settlement_report['total_processed']) * 100 if settlement_report['total_processed'] > 0 else 0
        ...
        self.logger.info(f"🎉 Robust Settlement Complete: {final_report['settled_bets']}/{final_report['total_pending']} bets settled")
