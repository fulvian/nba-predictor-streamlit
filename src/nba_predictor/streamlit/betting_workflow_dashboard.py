...
try:
    from nba_predictor.utils.robust_bet_settlement import (
        create_robust_settlement_system,
    )
...
try:
    from nba_predictor.streamlit.styling_system_safe import (
        NBAStylingSafe,
        apply_safe_styling,
        create_safe_hero_header,
        create_safe_section_header,
    )
...
from nba_predictor.streamlit.components.enhanced_predictions_dashboard import (
    render_enhanced_predictions_dashboard,
)

# Import REAL DATA Enhanced Bridge
from nba_predictor.streamlit.components.enhanced_prediction_bridge_real_data import (
...
def render_legacy_betting_analysis(game: Dict[str, Any], central_line: float):
    ...
    try:
        ...
        with st.container(border=True, height=150):
            ...
            with col3:
                st.metric(
                    "💵 Disponibile", f"€{bankroll_status['available_bankroll']:.2f}"
                )
            with col4:
        ...
        if "ml_prediction" in game:
            ...
            logger.info(
                f"✅ Found ML prediction embedded in game object: {ml_prediction.get('predicted_total', 'N/A')}"
            )
        elif (
            st.session_state.selected_game
            and "ml_prediction" in st.session_state.selected_game
        ):
            ml_prediction = st.session_state.selected_game["ml_prediction"]
            logger.info(
                f"✅ Found ML prediction in session_state.selected_game: {ml_prediction.get('predicted_total', 'N/A')}"
            )
        else:
            # Method 2: Check predictions_cache (might be lost due to session reset)
            if "predictions_cache" in st.session_state:
                logger.info(
                    f"🔍 DEBUG: Found predictions_cache with {len(st.session_state.predictions_cache)} items"
                )
                for (
                    cache_key,
                    prediction_data,
                ) in st.session_state.predictions_cache.items():
                    logger.info(f"   Cache key: {cache_key}")
                    if prediction_data:
                        logger.info(
                            f"   Prediction data keys: {list(prediction_data.keys())}"
                        )
                        logger.info(
                            f"   Teams: {prediction_data.get('home_team', 'N/A')} vs {prediction_data.get('away_team', 'N/A')}"
                        )
                        logger.info(
                            f"   Predicted total: {prediction_data.get('predicted_total', 'N/A')}"
                        )
                ...
                for (
                    ...
                    if (
                        prediction_data
                        and "home_team" in prediction_data
                        and "away_team" in prediction_data
                    ):
                        ...
                        logger.info(
                            f"🔍 DEBUG: Comparing {pred_home} vs {pred_away} with {game_home} vs {game_away}"
                        )
                        ...
                        if (pred_home == game_home and pred_away == game_away) or (
                            ...
                            logger.info(
                                f"✅ Found ML prediction in cache: {prediction_data.get('predicted_total', 'N/A')}"
                            )
                            break
            else:
                ...
                logger.info(
                    "🚀 Context7: No ML prediction found, generating real-time prediction..."
                )
                try:
                    from nba_predictor.streamlit.components.enhanced_prediction_bridge_real_data import (
                        get_enhanced_prediction_bridge_real_data,
                    ...
                    logger.info(
                        f"✅ Context7: Generated real ML prediction: {ml_prediction.get('predicted_total', 'N/A')}"
                    )
                    ...
                    logger.error(
                        f"❌ Context7: Failed to generate ML prediction: {str(e)}"
                    )
                    ml_prediction = None
        ...
        if ml_prediction and "predicted_total" in ml_prediction:
            ...
            if (
                "confidence_interval" in ml_prediction
                and ml_prediction["confidence_interval"]
            ):
            ...
            logger.info(
                f"🎯 Using ML distribution: μ={predicted_mu:.1f}, σ={predicted_sigma:.1f}"
            )
        else:
            # Fallback to mock only if ML prediction not available
            logger.warning(
                "⚠️ ML prediction not found, using mock distribution as fallback"
            )
            distribution = {
        ...
        with st.status(
            "🔄 Elaborazione algoritmi legacy di gestione rischio...", expanded=True
        ) as status:
            ...
            status.update(
                label="✅ Analisi Legacy Completa", state="complete", expanded=False
            )
        ...
        if distribution.get("source") == "ML_MODEL":
            st.success(
                "✅ **USANDO PREDIZIONI ML REALI** - Sistema completamente integrato!"
            )
            st.info(
                f"🧠 Predizione ML: {distribution['predicted_mu']:.1f} punti (±{distribution['predicted_sigma']:.1f})"
            )
        else:
            st.warning("⚠️ **USANDO DATI MOCK** - Predizioni ML non disponibili")

        st.success(
            f"✅ Trovate {len(value_bets)} VALUE bets su {len(opportunities)} linee analizzate"
        )
        ...
        st.subheader(
            "🎯 ANALISI COMPLETA SISTEMA LEGACY - Tutte le Quote e Probabilità"
        )
        ...
        for i, bet in enumerate(opportunities, 1):
            ...
            quality = risk_manager.calculate_quality_score(
                bet.get("edge", 0), bet.get("probability", 0), bet.get("odds", 1.0)
            )
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "LINEA": st.column_config.NumberColumn(
                    "Linea", format="%.1f", width="small"
                ),
                "QUOTA": st.column_config.NumberColumn(
                    "Quota", format="%.2f", width="small"
                ),
                "EDGE%": st.column_config.NumberColumn(
                    "Edge%", format="%.2f", width="small"
                ),
                "PROB%": st.column_config.NumberColumn(
                    "Prob%", format="%.1f", width="small"
                ),
                "IMPL%": st.column_config.NumberColumn(
                    "Impl%", format="%.1f", width="small"
                ),
                "TRUE%": st.column_config.NumberColumn(
                    "True%", format="%.1f", width="small"
                ),
                "QUALITY": st.column_config.NumberColumn(
                    "Quality", format="%.1f", width="small"
                ),
                "EDGE_S": st.column_config.NumberColumn(
                    "Edge Score", format="%.1f", width="small"
                ),
                "CONF": st.column_config.NumberColumn(
                    "Confidence", format="%.1f", width="small"
                ),
                "RISK_S": st.column_config.NumberColumn(
                    "Risk Score", format="%.1f", width="small"
                ),
                "CONS": st.column_config.NumberColumn(
                    "Consistency", format="%.1f", width="small"
                ),
                "KELLY%": st.column_config.NumberColumn(
                    "Kelly%", format="%.2f", width="small"
                ),
                "STAKE": st.column_config.NumberColumn(
                    "Stake", format="%.2f", width="small"
                ),
                "ROI%": st.column_config.NumberColumn(
                    "ROI%", format="%.1f", width="small"
                ),
        ...
        if optimal_bet:
            ...
            def highlight_recommendations(row):
                if "SCELTA SISTEMA" in row["CATEGORIA"]:
                    return ["background-color: #FFD700; font-weight: bold"] * len(
                        row
                    )  # Oro
            ...
            with col1:
                ...
                if STYLING_SAFE_AVAILABLE:
                    st.markdown(
                        create_safe_section_header(
                            "📊 Analysis Metrics",
                            "Key performance indicators for this betting opportunity",
                        ),
                        unsafe_allow_html=True,
                    )
                else:
                ...
                with col_metrics1:
                    ...
                    st.metric(
                        label="💰 Stake Amount", value=f"€{optimal_bet['stake']:.2f}"
                    )
                ...
                edge_value = optimal_bet["edge"] * 100
                if STYLING_SAFE_AVAILABLE:
                    value_indicator_html = NBAStylingSafe.create_safe_value_indicator(
                        edge_value
                    )
                            ...
                            try:
                                ...
                                with BettingDatabaseManager() as db_manager:
                                    ...
                                    if not game_id:
                                        ...
                                        game_date = (
                                            date.today()
                                        )  # Use today's date for manual games
                                    ...
                                    quality_result = (
                                        risk_manager.calculate_quality_score(
                                            optimal_bet["edge"],
                                    ...
                                    bet_analysis = BetAnalysis(
                                        ...
                                        quality_score=optimal_bet.get(
                                            "quality_score", 0.8
                                        ),
                                        edge_score=optimal_bet.get(
                                            "edge_score",
                                            min(optimal_bet["edge"] * 10, 1.0),
                                        ),
                                        confidence_score=optimal_bet.get(
                                            "confidence_score", 0.7
                                        ),
                                        risk_score=quality_result["risk_score"],
                                        consistency_score=optimal_bet.get(
                                            "consistency_score", 0.8
                                        ),
                                        kelly_fraction=quality_result["kelly_fraction"],
                                        stake=stake_override,
                                        roi=(optimal_bet["odds"] - 1) * 100,
                                        is_value=optimal_bet.get(
                                            "is_value", optimal_bet["edge"] > 0.02
                                        ),
                                    ...
                                    game_info = db_manager.get_game_from_database(
                                        game_id
                                    )
                                    is_played_game = game_info and game_info.get(
                                        "is_played", False
                                    )

                                    # Check for existing bets on this game
                                    existing_bets = (
                                        db_manager.check_existing_bets_for_game(game_id)
                                    )

                                    # Handle different scenarios
                                    if is_played_game:
                                        st.warning(
                                            "⚠️ **ATTENZIONE: Questo incontro è già stato concluso!**"
                                        )
                                        st.info(
                                            f"📊 Risultato finale: {game_info.get('final_home_score', '?')}-{game_info.get('final_away_score', '?')}"
                                        )
                                        ...
                                        if st.checkbox(
                                            "🎲 Procedi comunque (scommessa post-game)",
                                            help="Permette di piazzare scommesse su giochi già conclusi per testing",
                                        ):
                                            st.warning(
                                                "⚠️ Stai piazzando una scommessa su un incontro già concluso!"
                                            )

                                    elif existing_bets:
                                        # Game not played but has existing bets
                                        st.warning(
                                            f"⚠️ **Esistono già {len(existing_bets)} scommesse per questo incontro!**"
                                        )
                                        ...
                                        for bet in existing_bets:
                                            ...
                                            st.write(
                                                f"{status_color} {bet.bet_type} {bet.line} @ {bet.odds:.2f} - {bet.status.upper()} - €{bet.stake:.2f}"
                                            )
                                        ...
                                        action = st.radio(
                                            "Cosa vuoi fare?",
                                            options=[
                                                "Mantieni tutte le scommesse",
                                                "Sovrascrivi scommesse esistenti",
                                            ],
                                            index=0,
                                        ...
                                        if action == "Sovrascrivi scommesse esistenti":
                                            if not st.checkbox(
                                                ...
                                                help="Seleziona per confermare la sovrascrittura",
                                            ):
                                                st.stop()
                                            st.warning(
                                                "⚠️ Tutte le scommesse esistenti verranno cancellate e rimborsate!"
                                            )
                                    else:
                                        # Normal case - no existing bets
                                        pass

                                    # Context7: GENERATE REAL NBA GAME ID FOR SETTLEMENT
                                    try:
                                        # Generate NBA-standard game_id for settlement to work
                                        from datetime import date
                                        ...
                                        def get_nba_team_abbreviation(team_name):
                                            ...
                                            return team_mappings.get(
                                                team_name, team_name[:3].upper()
                                            )

                                        # Create real NBA game_id from current game
                                        game_date = date.today()
                                        home_abbr = get_nba_team_abbreviation(
                                            bet_analysis.home_team
                                        )
                                        away_abbr = get_nba_team_abbreviation(
                                        ...
                                        st.success(
                                            f"✅ Scommessa piazzata con successo! (Game ID: {real_game_id})"
                                        )
                                        analysis_id = f"BET_{int(time.time())}"

                                    except Exception as e:
                                        st.error(
                                            f"❌ Errore nel piazzare scommessa: {str(e)}"
                                        )
                                        st.stop()
                                    ...
                                    if existing_bets and not is_played_game:
                                        ...
                                        if (
                                            st.session_state.get("bet_action_choice")
                                            == "Sovrascrivi scommesse esistenti"
                                        ):
                                        ...
                                        bet_id = db_manager.place_bet(
                                            ...
                                            if st.session_state.get("bet_notes_input")
                                            else None,
                                        )
                                    ...
                                    if bet_id:
                                        ...
                                        st.session_state.last_bet_details = {
                                            "bet_id": bet_id,
                                            "stake": stake_override,
                                            "potential_return": stake_override
                                            * optimal_bet["odds"],
                                            "game": game,
                                            "bet_type": optimal_bet["type"],
                                            "line": optimal_bet["line"],
                                        ...
                                        st.success(
                                            f"✅ **SCOMMESSA PIAZZATA CON SUCCESSO!**"
                                        )
                                        st.info(f"📋 Bet ID: `{bet_id}`")
                                        st.info(f"💰 Importo: €{stake_override:.2f}")
                                        st.info(
                                            f"📊 Potenziale vincita: €{stake_override * optimal_bet['odds']:.2f}"
                                        )

                                        # Show bankroll status with error handling
                                        try:
                                            bankroll_status = (
                                                db_manager.get_bankroll_status()
                                            )
                                            st.info(
                                                f"💵 Bankroll attuale: €{bankroll_status['current_bankroll']:.2f}"
                                            )
                                            st.info(
                                                f"📊 Scommesse attive: {bankroll_status['pending_bets_count']}"
                                            )
                                        except Exception as bankroll_error:
                                            logger.warning(
                                                f"Bankroll status error: {bankroll_error}"
                                            )
                                        ...
                                        st.error(
                                            "❌ **ERRORE nel piazzamento della scommessa**"
                                        )
            ...
            with col3:
                ...
                try:
                    with BettingDatabaseManager() as db_manager:
                        ...
                        st.metric(
                            "📊 Scommesse Attive", bankroll_status["pending_bets_count"]
                        )
...
def setup_session_state() -> None:
    ...
    if st.session_state.debug_mode:
        logger.info(
            f"Session state initialized. Current step: {st.session_state.betting_workflow_step}"
        )
    ...
    try:
        ...
        if st.session_state.debug_mode:
            ...
            for game in games[:3]:  # Log first 3 games for debugging
                logger.info(
                    f"Game: {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')} - {game.get('date', 'Unknown')}"
                )
...
def _load_games_with_intelligent_caching(date_str: str, days_ahead: int) -> list:
    ...
    try:
        ...
        if cached_games:
            if st.session_state.debug_mode:
                logger.info(
                    f"✅ Data Store HIT: Found {len(cached_games)} games in persistent storage"
                )
                for game in cached_games[:2]:
                    logger.info(
                        f"   📦 Cached: {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')}"
                    )
            return cached_games
        else:
            if st.session_state.debug_mode:
...
def _check_data_store_for_games(
    data_store: UnifiedDataStore, date_str: str, days_ahead: int
) -> Optional[list]:
    """
    ...
    try:
        ...
        if st.session_state.debug_mode:
            logger.info(
                f"   🔍 Querying data store for games between {start_date_str} and {end_date_str}"
            )
...
def _fetch_games_from_apis_with_caching(date_str: str, days_ahead: int) -> list:
    ...
    try:
        ...
        games = data_provider.get_scheduled_games(
            days_ahead=days_ahead, specific_date=date_str
        )
...
def _update_data_store_with_games(
    data_store: UnifiedDataStore, games: list, date_str: str
):
    """
    ...
    try:
        ...
        # Save to data store
        # Use UnifiedDataStore to save games data
        data_store.save_games(df)

        if st.session_state.debug_mode:
            logger.info(f"   💾 Saved {len(game_records)} games to data store")
...
def select_game_callback(game_index):
    ...
    if game_index >= len(st.session_state.available_games):
        logger.warning(
            f"Game index {game_index} out of range (0-{len(st.session_state.available_games) - 1})"
        )
        return
    ...
    if st.session_state.debug_mode:
        logger.info(
            f"Selected game: {selected_game.get('away_team', 'Unknown')} @ {selected_game.get('home_team', 'Unknown')}"
        )
...
def select_nearby_game_callback(nearby_date, game_index, nearby_games_list):
    ...
    if st.session_state.debug_mode:
        logger.info(
            f"Select nearby game callback: date={nearby_date}, index={game_index}"
        )
    ...
    try:
        ...
        if st.session_state.debug_mode:
            logger.info(
                f"Updated date to {nearby_date} and selected game at index {game_index}"
            )
...
def render_games_schedule_step(data_provider: NBADataProvider) -> None:
    ...
    if st.session_state.get("available_games"):
        ...
        exact_date_games = [
            game for game in games if game.get("date") == selected_date_str
        ]
        ...
        if exact_date_games:
            st.success(
                f"✅ Found {len(exact_date_games)} games for {selected_date_str}"
            )
                ...
                for nearby_date, nearby_games in sorted(games_by_date.items()):
                    with st.expander(f"📅 {nearby_date} ({len(nearby_games)} games)"):
                        for i, game in enumerate(nearby_games):
                            ...
                            with col1:
                                st.write(
                                    f"🏀 {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')}"
                                )
                                st.write(
                                    f"🕐 {convert_game_time_to_et(game)} | 📊 {game.get('status', 'Scheduled')}"
                                )
                            with col2:
    ...
    if st.session_state.debug_mode:
        with st.expander("🔍 Debug Information", expanded=False):
            ...
            debug_info = {
                ...
                "Available Games Count": len(st.session_state.available_games)
                if st.session_state.available_games
                else 0,
                "Selected Game": f"{st.session_state.selected_game.get('away_team', 'N/A')} @ {st.session_state.selected_game.get('home_team', 'N/A')}"
...
def render_game_analysis_step() -> None:
    ...
    st.info(
        f"**Selected Game**: {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')} - {game.get('date', 'Unknown')}"
    )
    ...
    try:
        ...
        render_enhanced_predictions_dashboard(
            data_store, sync_engine, st.session_state.selected_game
        )
...
def render_betting_lines_step(data_provider: NBADataProvider) -> None:
    ...
    if st.session_state.bet_placed == "success" and st.session_state.last_bet_details:
        ...
        with col3:
            st.metric(
                "🏆 Potenziale Vincita", f"€{bet_details['potential_return']:.2f}"
            )
        ...
        st.info(
            f"🏀 **Partita**: {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')} - {game.get('date', 'Unknown')}"
        )
    ...
    with tab_guide:
        ...
        st.success(
            "💡 **Pro Tip**: The system uses proven algorithms from professional betting analysis with comprehensive risk management.",
            icon="💡",
        )
...
def render_comprehensive_bets_view():
    ...
    try:
        ...
        with col_bank2:
            st.metric(
                "Scommesse Pendenti", bankroll_status.get("pending_bets_count", 0)
            )
        ...
        with col_update1:
            if st.button(
                ...
                help="Auto-settle tutte le scommesse per giochi conclusi",
            ):
                with st.spinner("Aggiornamento risultati in corso..."):
                    try:
                        # Get all games with final scores - simplified approach
                        try:
                            # Use UnifiedDataStore to get games data
                            games_data = data_store.get_games_data(
                                date_range=(
                                    date.today() - timedelta(days=30),
                                    date.today(),
                                ),
                                include_final_only=True,
                            )

                            # Convert to expected format
                            games_result = []
                            if games_data is not None:
                                for row in games_data.iter_rows():
                                    games_result.append(row)
                        ...
                        if total_settled > 0:
                            st.success(
                                f"✅ {total_settled} scommesse auto-settled con successo!"
                            )
                            st.rerun()
                        else:
        ...
        with col_update2:
            ...
            # Use the existing data_store from the outer scope
            games_path = data_store.games_dir + "/*.parquet"
            query = """
                SELECT DISTINCT pb.game_id, COUNT(*) as bet_count
                FROM bets pb
                LEFT JOIN read_parquet('{}') g ON (
                ...
                LIMIT 5
            """.format(games_path)
            pending_games = db_manager.conn.execute(query).fetchall()

            if pending_games:
                st.warning(
                    f"⚠️ {len(pending_games)} giochi conclusi con scommesse pendenti"
                )
        ...
        with col_update3:
            ...
            """.format(games_path)
            today_games = db_manager.conn.execute(query).fetchone()

            if today_games and today_games[0] > 0:
                st.info(
                    f"📅 Giochi di oggi: {today_games[1]}/{today_games[0]} conclusi"
                )
            else:
...
def render_pending_bets_table(
    pending_bets: List[PlacedBet], db_manager: BettingDatabaseManager
):
    ...
    for i, bet in enumerate(pending_bets):
        ...
        with st.expander(
            f"💎 {bet.bet_type} {bet.line} @ {odds_float:.2f} - €{stake_float:.2f}"
        ):
            col1, col2, col3 = st.columns(3)
            ...
            with col2:
                ...
                if game_info:
                    if game_info.get("is_played", False):
                        st.warning(
                            f"🏀 Gioco concluso! Score: {game_info.get('final_home_score', '?')}-{game_info.get('final_away_score', '?')}"
                        )

                        if st.button(
                            f"🔄 Auto-settle {bet.bet_id}", key=f"settle_{bet.bet_id}"
                        ):
                            with st.spinner("Auto-settling bet..."):
                                settled_count = (
                                    db_manager.update_game_results_from_scores(
                                        bet.game_id,
                                ...
                                if settled_count > 0:
                                    st.success(
                                        f"✅ Scommessa auto-settled con successo!"
                                    )
                                    st.rerun()
                                else:
            ...
            with col3:
                ...
                new_notes = st.text_area(
                    "📝 Note", value=current_notes, key=f"notes_{bet.bet_id}", height=80
                )
...
def render_settled_bets_table(
    settled_bets: List[PlacedBet], db_manager: BettingDatabaseManager
):
    ...
    for bet in settled_bets:
        ...
        status_color = {"won": "🟢", "lost": "🔴", "void": "🟡", "cancelled": "⚫"}.get(
            bet.status, "⚪"
        )

        with st.expander(
            f"{status_color} {bet.bet_type} {bet.line} @ {odds_float:.2f} - {bet.status.upper()}"
        ):
            col1, col2 = st.columns(2)
            ...
            with col1:
                ...
                st.write(f"**Stake:** €{stake_float:.2f}")
                st.write(
                    f"**Risultato:** €{result_float:.2f}"
                    if bet.result_amount
                    else "**Risultato:** N/A"
                )

                profit_color = (
                    "🟢" if profit_float > 0 else "🔴" if profit_float < 0 else "⚪"
                )
                st.write(
                    f"**P/L:** {profit_color} €{profit_float:.2f}"
                    if bet.profit_loss
                    else "**P/L:** N/A"
                )
            ...
            with col2:
                ...
                if bet.settled_at:
                    st.write(
                        f"**Conclusa:** {bet.settled_at.strftime('%Y-%m-%d %H:%M')}"
                    )
    ...
    try:
        ...
        if settlement_result["total_pending"] > 0:
            if settlement_result["settled_bets"] > 0:
                st.success(
                    f"✅ **Robust Settlement Complete**: {settlement_result['settled_bets']}/{settlement_result['total_pending']} bets settled automatically!"
                )
                    ...
                    with st.expander("📋 Settlement Details", expanded=False):
                        for detail in settlement_result["details"]:
                            if detail["result"] == "settled":
                                st.success(
                                    f"🏀 Bet {detail['bet_id']}: Game {detail.get('nba_game_id', detail.get('bet_id'))} settled with score {detail['final_score']}"
                                )
                            elif detail["result"] == "failed":
                                st.warning(
                                    f"⚠️ Bet {detail['bet_id']}: {detail.get('reason', 'Unknown error')}"
                                )
            else:
                st.info(
                    f"ℹ️ **No Settlements**: {settlement_result['total_pending']} pending bets processed, but none could be settled"
                )

            # Mostra summary
            if settlement_result["failed_settlements"] > 0:
                st.warning(
                    f"⚠️ {settlement_result['failed_settlements']} bets could not be settled (see details)"
                )
...
def render_all_bets_table(
    all_bets: List[PlacedBet], db_manager: BettingDatabaseManager
):
...
def render_data_store_management(db_manager: BettingDatabaseManager):
    ...
    with col_op1:
        if st.button(
            "🔄 Sync Data Store", help="Synchronize and validate data integrity"
        ):
...
def render_betting_workflow_dashboard() -> None:
    ...
    try:
        ...
        if STYLING_SAFE_AVAILABLE:
            ...
            st.markdown(
                "Advanced Analytics & Risk Management System with Enhanced ML Engine"
            )
            ...
            try:
                ...
                if health_status.get("model_status", {}).get("is_trained", False):
                    st.success(
                        "✅ **Enhanced ML System: OPERATIONAL** - Production-ready predictions"
                    )
                else:
                    st.warning(
                        "⚠️ **Enhanced ML System: TRAINING** - Initializing model..."
                    )
                ...
                with st.sidebar:
                    ...
                    st.write(
                        "**Model Version:**",
                        f"v{health_status.get('model_status', {}).get('model_version', 'N/A')}",
                    )
                    st.write(
                        "**Features:**",
                        f"{health_status.get('model_status', {}).get('feature_count', 0)} engineered",
                    )
                    st.write(
                        "**Monitoring:**",
                        "✅ Active"
                        if health_status.get("monitoring_status", {}).get("status")
                        == "active"
                        else "❌ Inactive",
                    )
                    ...
                    # Quality indicator
                    model_quality = health_status.get("model_status", {}).get(
                        "is_trained", False
                    )
                    if model_quality:
                        st.success(
        ...
        with st.sidebar:
            ...
            if STYLING_SAFE_AVAILABLE:
                st.markdown(
                    create_safe_section_header(
                        "📋 Workflow Progress",
                        "Complete each step to unlock advanced analysis",
                    ),
                    unsafe_allow_html=True,
                )
            else:
            ...
            with col_steps1:
                if STYLING_SAFE_AVAILABLE:
                    step1_html = NBAStylingSafe.create_safe_step_indicator(
                        1, current_step, "📅 Game Schedule"
                    )
            ...
            with col_steps2:
                if STYLING_SAFE_AVAILABLE:
                    step2_html = NBAStylingSafe.create_safe_step_indicator(
                        2, current_step, "📊 Game Analysis"
                    )
            ...
            with col_steps3:
                if STYLING_SAFE_AVAILABLE:
                    step3_html = NBAStylingSafe.create_safe_step_indicator(
                        3, current_step, "💰 Betting Lines"
                    )
            ...
            with col_steps4:
                if STYLING_SAFE_AVAILABLE:
                    step4_html = NBAStylingSafe.create_safe_step_indicator(
                        4, current_step, "💎 Gestione Scommesse"
                    )
            ...
            if st.button(
                "💎 Gestione Scommesse", help="Visualizza e gestisci tutte le scommesse"
            ):
        ...
        st.divider()
        st.caption(
            "NBA Betting Workflow Dashboard | Real-time NBA Data & ML Analysis | Context7 Best Practices"
        )
...
if __name__ == "__main__":
    render_betting_workflow_dashboard()
