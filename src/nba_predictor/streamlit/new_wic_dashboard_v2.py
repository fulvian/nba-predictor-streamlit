...
def get_initialized_managers():
    ...
    # Initialize Pipeline (One-time load)
    ...
    return ds, db, pipeline


data_store, db_manager, pipeline = get_initialized_managers()
...
def render_prediction_summary_v2(prediction: Dict[str, Any]):
    """
    Render a modern, card-based prediction summary with MOE CONSENSUS visualization.
    """
    if not prediction:
        return
    ...
    if game and (not home_team or not away_team):
...
<div class="game-card">
    <div class="card-header">
        ...
        <div class="game-status" style="display:flex;align-items:center;gap:6px;">{assets.ICON_BASKETBALL.replace('width="24"', 'width="20"').replace('height="24"', 'height="20"')} MoE Hybrid Analysis</div>
    </div>
...
"""
    ...
    # --- REASONING CORTEX (New for V2) ---
    st.markdown(f"### 🧠 Consensus Reasoning Cortex", unsafe_allow_html=True)

    # Determine color for risk
    risk_color = (
        "red"
        if "high" in str(risk_level).lower()
        else "orange"
        if "med" in str(risk_level).lower()
        else "green"
    )

    st.info(f"**Strategic Reasoning:** {reasoning}")
    ...
    with st.expander("Advanced Analytics & Factors", expanded=False):
        ...
        with cols[0]:
            ...
            if momentum:
                # Sanitize complex types for PyArrow compatibility
                import json
                ...
                for k, v in momentum.items():
                    if isinstance(v, (dict, list)):
                        ...
                        sanitized[k] = v
                st.dataframe(pd.DataFrame([sanitized]).T, width="stretch")
            else:
                st.write("No specific momentum factors.")

        with cols[1]:
            st.markdown(
                f"#### {assets.ICON_BRAIN} Prediction Statistics",
                unsafe_allow_html=True,
            )
            st.metric("Std Error", f"{prediction.get('standard_error', 0):.2f}")
            st.metric("Over Prob", f"{prediction.get('over_probability', 0):.1%}")
            st.metric("Under Prob", f"{prediction.get('under_probability', 0):.1%}")
...
def render_step_2_predictor():
    ...
    if run_analysis and not prediction:
        with st.spinner("Running Advanced ML Analysis..."):
            try:
                ...
                # CI = mean +/- 1.96 * stderr -> Range = 3.92 * stderr
                ci_range = (
                    prediction_result.confidence_interval[1]
                    - prediction_result.confidence_interval[0]
                )
                std_error = ci_range / 3.92

                # Map UnifiedPredictionResult to Dashboard Dictionary Format
                prediction = {
                    ...
                    "roster_changes": prediction_result.roster_changes,
                    "momentum": prediction_result.player_momentum,
                    "factors": prediction_result.four_factors_analysis,
                    "shap_explanation": prediction_result.shap_explanation,
                }
    ...
    # Display Prediction
    if prediction:
        render_prediction_summary_v2(prediction)
    else:
