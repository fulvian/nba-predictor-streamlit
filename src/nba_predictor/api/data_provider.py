...
class CacheEntry:
    ...
    cache_duration_seconds: int = (
        72  # Cache ottimizzato: 1.2 minuti per 5 richieste/minuto
    )
...
class GameCache:
    ...
    def set(
        self, cache_key: str, data: List[Dict[str, Any]], duration_seconds: int = 72
    ) -> None:
        """Salva dati nella cache con durata ottimizzata."""
        entry = CacheEntry(
            data=data, timestamp=datetime.now(), cache_duration_seconds=duration_seconds
        )
...
class NBADataProvider:
    ...
    def __init__(self):
        ...
        # Import Normalizer
        try:
            from ..utils.team_normalizer import TeamNameNormalizer
        ...
        self.odds_api_key = os.getenv(
            "THE_ODDS_API_KEY", "d01e24415744d440168e0a489f233aac"
        )
        ...
        self.team_name_to_info = {
            team["full_name"]: team for team in self.nba_teams_info
        }
    ...
    def _get_odds_api_games(self, days_ahead=7):
        ...
        try:
            print(
                f"   🎰 The Odds API: Richiesta partite future (prossimi {days_ahead} giorni)..."
            )
            ...
            response = self.odds_session.get(
                url, params=params, headers=self.odds_headers, timeout=15
            )
    ...
    def _get_nba_official_games(
        self, days_ahead: int = 7, specific_date: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            print(
                f"   🏀 NBA Official API: Richiesta partite ufficiali NBA.com (cache miss)..."
            )
    ...
    def _get_ball_dont_lie_games(
        ...
        try:
            ...
            # Check cache first
            cache_key = (
                f"bdl_{start_date.strftime('%Y-%m-%d')}_{end_date.strftime('%Y-%m-%d')}"
            )
            cached_games = game_cache.get(cache_key)
            if cached_games:
                return cached_games
            ...
            print(
                f"   🏀 BallDontLie API: Richiesta partite ufficiali da {start_date} a {end_date}..."
            )
            ...
            print(
                f"   ✅ BallDontLie API: {len(formatted_games)} partite ufficiali trovate"
            )
    ...
    def _extract_main_odds(self, game):
        ...
        """
        odds_data = {"moneyline": {}, "spread": {}, "total": {}}
    ...
    def get_team_roster(
        self, team_id: int, season: str = None
    ...
    def get_team_stats_for_game(
        ...
        try:
            ...
            for stats in [home_stats, away_stats]:
                stats["field_goal_percentage"] = max(
                    0.35, min(0.55, stats["field_goal_percentage"])
                )
                stats["three_point_percentage"] = max(
                    0.25, min(0.45, stats["three_point_percentage"])
                )
                stats["free_throw_percentage"] = max(
                    0.65, min(0.90, stats["free_throw_percentage"])
                )
