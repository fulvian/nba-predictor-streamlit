...
# Use production models instead of deprecated duplicates
from nba_predictor.ensemble.nba_ensemble_predictor import (
    NBAEnsemblePredictor as NBAEnsembleModel,
)
from nba_predictor.core.unified_ml_interface import UnifiedMLInterface
from nba_predictor.core.data_store import UnifiedDataStore


# ModelConfig is deprecated, use direct configuration
class ModelConfig:
    """Configuration for NBA ML models (deprecated)."""

    def __init__(self, **kwargs):
        self.xgb_n_estimators = kwargs.get("xgb_n_estimators", 100)
        self.rf_n_estimators = kwargs.get("rf_n_estimators", 100)
        self.test_size = kwargs.get("test_size", 0.2)


# NBAFeatureEngineer is deprecated, use simplified approach
class NBAFeatureEngineer:
    """Simplified feature engineer for API compatibility."""

    def __init__(self, data_store):
        self.data_store = data_store

    def process_team_features(self, season):
        """Process team features (simplified)."""
        try:
            # Use data store to get team data
            return self.data_store.get_team_data(season)
        except:
            return None

    def process_injury_features(self, season):
        """Process injury features (simplified)."""
        try:
            return self.data_store.get_injury_data(season)
        except:
            return None
...
class PredictionRequest(BaseModel):
    ...
    prediction_type: str = Field(
        default="classification", description="classification or regression"
    )
...
class PredictionResponse(BaseModel):
    ...
    shap_explanations: Optional[Dict[str, float]] = Field(
        None, description="SHAP feature explanations"
    )
...
class PredictionService:
    ...
    def initialize_models(self):
        ...
        try:
            ...
            model_config = ModelConfig(
                xgb_n_estimators=100, rf_n_estimators=100, test_size=0.2
            )
    ...
    def process_game_data(self, game_data: GameData) -> Dict[str, Any]:
        ...
        try:
            ...
            team_features = self.feature_engineer.process_team_features(
                game_data.season
            )
            ...
            combined_features = {
                "home_score_diff": float(
                    home_features.select("PTS").mean()
                    - away_features.select("PTS").mean()
                )
            ...
            try:
                injury_features = self.feature_engineer.process_injury_features(
                    game_data.season
                )
                if injury_features is not None and len(injury_features) > 0:
                    home_injury = injury_features.filter(
                        pl.col("TEAM_ID") == game_data.home_team_id
                    )
                    away_injury = injury_features.filter(
                        pl.col("TEAM_ID") == game_data.away_team_id
                    )

                    if len(home_injury) > 0 and len(away_injury) > 0:
                        combined_features["home_injury_impact"] = float(
                            home_injury.select("INJURY_IMPACT").mean()
                        )
                        combined_features["away_injury_impact"] = float(
                            away_injury.select("INJURY_IMPACT").mean()
                        )
                        combined_features["injury_diff"] = (
                            combined_features["home_injury_impact"]
                            - combined_features["away_injury_impact"]
                        )
            except Exception as e:
    ...
    def _create_mock_features(self, game_data: GameData) -> Dict[str, Any]:
        ...
        logger.info(
            f"📊 Created mock features for {game_data.home_team_name} vs {game_data.away_team_name}"
        )
        return mock_features

    def make_prediction(
        self, features: Dict[str, Any], prediction_type: str = "classification"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                hasattr(self.ensemble_model, "models")
                and len(self.ensemble_model.models) > 1
            ):
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        welcome_msg = WebSocketMessage(
            ...
            data={
                "status": "connected",
                "message": "Connected to NBA Prediction WebSocket",
            },
    ...
    async def broadcast_prediction(
        self, game_data: GameData, prediction_result: Dict[str, Any]
    ):
...
class NBAPredictionAPI:
    ...
    def _setup_routes(self):
        ...
        async def predict_game(request: PredictionRequest):
            ...
            logger.info(
                f"🎯 Prediction request: {request.game.home_team_name} vs {request.game.away_team_name}"
            )
            ...
            if not self.prediction_service.ensemble_model:
                ...
                if not success:
                    raise HTTPException(
                        status_code=500, detail="Failed to initialize ML models"
                    )

            # Check cache
            cache_key = self.prediction_service.get_cache_key(
                request.game, request.prediction_type
            )
            ...
            prediction_result = self.prediction_service.make_prediction(
                features, request.prediction_type
            )

            # Get SHAP explanations if requested
            shap_explanations = None
            if request.include_shap:
                shap_explanations = self.prediction_service.get_shap_explanations(
                    features
                )

            # Cache result
            self.prediction_service.prediction_cache[cache_key] = {
                **prediction_result,
                "timestamp": datetime.now().isoformat(),
            ...
            await self.websocket_manager.broadcast_prediction(
                request.game,
                {**prediction_result, "shap_explanations": shap_explanations},
            )

            logger.info(
                f"✅ Prediction completed: {prediction_result['prediction']:.3f} (confidence: {prediction_result['confidence']:.3f})"
            )
        ...
        async def models_status():
            ...
            models_ready = (
                self.prediction_service.ensemble_model is not None
                and len(self.prediction_service.ensemble_model.models) > 0
            )

            feature_engineer_ready = (
                self.prediction_service.feature_engineer is not None
            )

            return {
                "status": "ready"
                if models_ready and feature_engineer_ready
                else "initializing",
                "models_loaded": len(self.prediction_service.ensemble_model.models)
                if models_ready
                else 0,
        ...
        async def initialize_models():
            ...
            if success:
                await self.websocket_manager.broadcast_status(
                    "ready", "ML models initialized successfully"
                )
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to initialize models"
                )
    ...
    def run(self):
        """Run the API server."""
        logger.info(
            f"🚀 Starting NBA Prediction API on {self.config.host}:{self.config.port}"
        )
    ...
    def run_websocket_server(self):
        ...
        logger.info(
            f"🌐 Starting WebSocket server on {self.config.host}:{self.config.websocket_port}"
        )
...
def create_app() -> NBAPredictionAPI:
    ...
    # Try to initialize models in background (only if event loop is running)
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(api.prediction_service.initialize_models())
    except RuntimeError:
        # No event loop running, models will be initialized on first request
        pass
